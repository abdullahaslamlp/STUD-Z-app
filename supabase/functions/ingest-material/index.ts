...
serve(async (req) => {
  ...
  try {
    ...
    // Extract raw text from the file (PDF/DOCX) so we can send text-only to the AI gateway
    ...
    // Cap input length to avoid token limits and timeouts (~120k chars ≈ ~30k tokens)
